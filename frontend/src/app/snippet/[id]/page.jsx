...
export default function SnippetDetailPage() {
  ...
  useEffect(() => {
    ...
    const fetchSnippet = async () => {
      try {
        const res = await fetch(`http://localhost:5000/api/snippet/getbyid/${id}`);
        ...
        console.error(err);
        setError(err.message || 'Could not retrieve snippet.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleLike = async () => {
    if (!user) {
      // Redirect to login
      router.push(`/login?redirect=/snippet/${id}`);
      return;
    }
    ...
    setLikeLoading(true);
    try {
      const res = await fetch(`http://localhost:5000/api/snippet/like/${id}`, {
        method: 'POST',
        headers: {
    ...
    return (
      <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col items-center justify-center p-4">
        <div className="text-center space-y-4">
          ...
          <p className="text-slate-400">{error || 'Snippet could not be found.'}</p>
          <button 
            onClick={() => router.push('/')}
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="flex items-center gap-1 text-xs text-slate-400">
                {snippet.isPublic ? (
                  ...
                  <><Lock className="w-3.5 h-3.5 text-yellow-500" /> Private</>
                )}
              </span>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 bg-slate-900/40 border border-slate-800 rounded-xl p-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm font-semibold text-slate-200">{snippet.views + 1}</p>
            </div>
          </div>
